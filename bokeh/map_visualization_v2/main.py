...
fish_data = pd.read_csv('bokeh/data/NCCA/ncca2010_ecological_fish_tissue_contaminant_data.csv')
location_data = pd.read_csv('bokeh/data/NCCA/assessed_ncca2010_siteinfo.revised.06212016.csv')

# Add site location coordinates
...
# Pull out PCB names from data set and sort
OPTIONS = list(set(fish_data.loc[fish_data['PARAMETER_CAT']=='PCB', 'PARAMETER']))
OPTIONS.sort()

# Initialize multichoice widget
multi_choice = MultiChoice(value=['PCB141'], options=OPTIONS, height=500)

# Initialize basemap
map_options = GMapOptions(lat=40.2861, lng=-97.7394, map_type="roadmap", zoom=4)

# Filter fish data on 
map_data = fish_data.loc[(fish_data['PARAMETER'].isin(multi_choice.value)) & (fish_data['RESULT']>0), 
...
colors = linear_cmap(field_name='result', palette=cc.fire, low=min(map_data['RESULT']), high=max(map_data['RESULT']))
color_mapper = LinearColorMapper(palette=cc.fire, low=min(map_data['RESULT']), high=max(map_data['RESULT']))

p.circle(x="lon", y="lat", size=10, line_color='white', fill_color=colors, fill_alpha=0.8, source=source)
p.add_layout(ColorBar(color_mapper=color_mapper, border_line_color=None, height=200, title="ng/wet g"),
             'right')

def select_data():

    if len(multi_choice.value)>0:
        map_data = fish_data.loc[(fish_data['PARAMETER'].isin(multi_choice.value)) & (fish_data['RESULT']>0), 
                                 ['SITE_ID', 'LAT_DD', 'LON_DD', 'PARAMETER', 'RESULT', 'UNITS']]
...
doc.add_root(row(column(multi_choice, width=200), p))
